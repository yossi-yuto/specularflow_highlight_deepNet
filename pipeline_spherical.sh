...
for iter in {1,2,3}; do
    ...
    CUDA_VISIBLE_DEVICES=${DEVICE_NUM} python main.py -dataset_path data/spherical_mirror_dataset -mode refine -write_dir ${write_dir}/ex_${iter} -test_mold_type ${kind} --train --eval
    CUDA_VISIBLE_DEVICES=${DEVICE_NUM} python main.py -dataset_path data/spherical_mirror_dataset -mode pmd -write_dir ${write_dir}/ex_${iter} -test_mold_type ${kind} --train --eval
done
